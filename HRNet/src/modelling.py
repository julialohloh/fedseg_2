...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.toOneVector = nn.Conv2d(19,1,kernel_size=1,stride=1,padding=0)
...
def _hrnet(modelcfg,**kwargs):
    """function to initialise model HighResolutionNet

    Args:
        modelcfg (dict): HRNET_18 model configs

    Returns:
        model:  HRNET_18 model
    """
#     from config.hrnet_config import MODEL_CONFIGS
# model = HighResolutionNet(MODEL_CONFIGS[arch], **kwargs)
    model = HighResolutionNet(modelcfg, **kwargs)
    # model.init_weights(modelcfg['PRETRAINED'])
    # if pretrained:
