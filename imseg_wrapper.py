...
class FedSeg(
    ...
    def load_patches(
        ...
        """
        patch_coords = self._perform_patching(image)
        for paired_points in patch_coords:
    ...
    def reassemble(
        ...
        """
        num_patches_height = math.ceil(self.image_height / self.patch_height)
        num_patches_width = math.ceil(self.image_width / self.patch_width)
        reconstructed_image = torch.zeros(
            (
                self.patch_height * num_patches_height,
                self.patch_width * num_patches_width,
                self.unet_channels,
            )
        )
        ...
        for i in range(num_patches_height):
            for j in range(num_patches_width):
                ...
                reconstructed_image[
                    (i * self.patch_height) : (
                        (i * self.patch_height) + self.patch_height
                    ),
                    (j * self.patch_width) : (
                        (j * self.patch_width) + self.patch_width
                    ),
                    :,
                ] = a
    ...
    def forward(self, image: PatchedImage) -> torch.Tensor:
        ...
        for patch in patches:
            ...
            hwc_image = torch.permute(patch, (1, 2, 0))
            print(f"PATCH:{hwc_image.shape}")
            # stacked_tensor = dask.delayed(self.forward_patch)(self, patch)
            stacked_tensor = self.forward_patch(hwc_image)
