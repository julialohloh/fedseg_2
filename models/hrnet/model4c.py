...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.toOneVector = nn.Conv2d(3,1,kernel_size=1,stride=1,padding=0)
...
class HRNetRefine(HighResolutionNet):
    ...
    def forward(self, x):
        """Forward function."""
        if x.size(dim=1) != int(4):
            raise ValueError("input channel must be 4")
        else:
            pass
        img = x[:,:3,...]
        mask = x[:,3:,...]
...
def _hrnet(modelcfg, **kwargs):
    PATH = 'worker/pretrain_model/hrnet_w18_small_model_v2.pth'
    model = HRNetRefine(modelcfg, **kwargs)
